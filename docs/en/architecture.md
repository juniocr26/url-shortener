...
- Local bind mounts for Redis and each Cassandra node.
- Cassandra password authentication and configured keyspace bootstrap.
...
- Data is stored in `.dockerized-redis/`.
- Restarts should preserve the local counter state as long as that directory is kept.
...
The configured keyspace is created during bootstrap with replication factor 3 in one local datacenter using `NetworkTopologyStrategy` and the configured datacenter name.
...
No application tables are created in this milestone because the access patterns and data model have not been decided yet.
...
The Compose file publishes local loopback ports for the application, Redis, and `cassandra-1`. `cassandra-2` and `cassandra-3` are only reachable inside the Docker network.
